...
public enum SSMExceptionCode {
	...
	private SSMExceptionCode(String code, String desin, String desout) {
		this.code = code;
		this.desin = desin;
		this.desout = desout;
	}
	...
	public static SSMExceptionCode getByCode(String code) {
		if (code != null && !"".equals(code.trim())) {
			for (SSMExceptionCode mnum : values()) {
	 ...
	 * (Add by Daniel Kong)
	 *
	 * @param SSMExceptionCode 异常的枚举值
	 * @return 提示给客户端的异常信息内容
	 */
	public static String getExceptionMsg(SSMExceptionCode SSMExceptionCode) {
		return ("[" + SSMExceptionCode.getCode() + "]" + SSMExceptionCode.getDesout());
	}
}
